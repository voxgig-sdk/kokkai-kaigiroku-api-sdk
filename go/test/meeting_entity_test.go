...
func TestMeetingEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set KOKKAI_KAIGIROKU_API_TEST_MEETING_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		meetingRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.meeting")))
...
func meetingBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("KOKKAI_KAIGIROKU_API_TEST_MEETING_ENTID")
	...
	idmapResolved := core.ToMapAny(env["KOKKAI_KAIGIROKU_API_TEST_MEETING_ENTID"])
