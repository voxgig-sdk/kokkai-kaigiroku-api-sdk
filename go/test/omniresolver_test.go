...
import (
	...
	omni "github.com/voxgig-sdk/kokkai-kaigiroku-api-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.KokkaiKaigirokuApiError); is {
				out := map[string]any{
					"name":    "KokkaiKaigirokuApiError",
...
func omniCtx(arg any, client *sdk.KokkaiKaigirokuApiSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.KokkaiKaigirokuApiSDK); is {
			client = live
