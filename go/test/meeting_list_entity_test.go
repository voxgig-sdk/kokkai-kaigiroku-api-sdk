...
func TestMeetingListEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		meetingListRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.meeting_list")))
...
func meeting_listBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("KOKKAI_KAIGIROKU_API_TEST_MEETING_LIST_ENTID")
	...
	idmapResolved := core.ToMapAny(env["KOKKAI_KAIGIROKU_API_TEST_MEETING_LIST_ENTID"])
