...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("KOKKAIKAIGIROKUAPI_TEST_LIVE") == "TRUE" ||
		os.Getenv("KOKKAIKAIGIROKUAPI_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("KOKKAIKAIGIROKUAPI_TEST_EXPLAIN"); explain != "" {
