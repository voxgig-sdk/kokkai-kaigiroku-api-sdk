...
The Golang SDK for the KokkaiKaigirokuApi API. Provides an entity-oriented interface using standard Go conventions — no generics required, data flows as `map[string]any`.
...
```go
    result, err := client.Meeting(nil).List(nil, nil)
    if err != nil {
        panic(err)
    }
