...
class TestSpeechDirect:

    def test_should_direct_list_speech(self):
        setup = _speech_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-speech", "live" if setup["live"] else "unit")
        if _skip:
...
def _speech_direct_setup(mockres):
