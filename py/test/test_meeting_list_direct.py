...
class TestMeetingListDirect:

    def test_should_direct_list_meeting_list(self):
        setup = _meeting_list_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-meeting_list", "live" if setup["live"] else "unit")
        if _skip:
...
def _meeting_list_direct_setup(mockres):
    ...
    if live:
        ...
        client = KokkaiKaigirokuApiSDK(merged_opts)
        return {
            "client": client,
            "calls": calls,
            "live": True,
