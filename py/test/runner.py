# KokkaiKaigirokuApi SDK test runner
...
class KokkaiKaigirokuApiTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            KokkaiKaigirokuApiTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = KokkaiKaigirokuApiTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = KokkaiKaigirokuApiTestRunner.getenv("KOKKAI_KAIGIROKU_API_TEST_LIVE")
        override = KokkaiKaigirokuApiTestRunner.getenv("KOKKAI_KAIGIROKU_API_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = KokkaiKaigirokuApiTestRunner.getenv(key)
        ...
        explain = KokkaiKaigirokuApiTestRunner.getenv("KOKKAI_KAIGIROKU_API_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["KOKKAI_KAIGIROKU_API_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if KokkaiKaigirokuApiTestRunner._test_control is not None:
            return KokkaiKaigirokuApiTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                KokkaiKaigirokuApiTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            KokkaiKaigirokuApiTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = KokkaiKaigirokuApiTestRunner.load_test_control()
...
def load_env_local():
    KokkaiKaigirokuApiTestRunner.load_env_local()


def env_override(m):
    return KokkaiKaigirokuApiTestRunner.env_override(m)


def entity_data(v):
    return KokkaiKaigirokuApiTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return KokkaiKaigirokuApiTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return KokkaiKaigirokuApiTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return KokkaiKaigirokuApiTestRunner.load_test_control()


def live_client_options():
    return KokkaiKaigirokuApiTestRunner.live_client_options()


def live_delay_ms():
    return KokkaiKaigirokuApiTestRunner.live_delay_ms()
