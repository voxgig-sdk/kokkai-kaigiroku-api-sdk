# KokkaiKaigirokuApi Python SDK



The Python SDK for the KokkaiKaigirokuApi API — an entity-oriented client following Pythonic conventions.
...
release tag (`py/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/kokkai-kaigiroku-api-sdk/releases)) or
...
| `pdfURL` | `str` | 会議録PDF表示画面のURL |
| `searchObject` | `str` | 検索対象箇所（議事冒頭・本文） |
| `session` | `int` | 国会回次 |
| `speechRecord` | `list` |  |
...
| `session` | `int` | 国会回次 |
| `speaker` | `str` | 発言者名 |
| `speakerGroup` | `str` | 発言者所属会派 |
| `speakerPosition` | `str` | 発言者肩書き |
| `speakerRole` | `str` | 発言者役割 |
| `speakerYomi` | `str` | 発言者よみ |
| `speech` | `str` | 発言 |
| `speechID` | `str` | 発言ID |
...
```
py/
├── kokkaikaigirokuapi_sdk.py         -- Main SDK module
