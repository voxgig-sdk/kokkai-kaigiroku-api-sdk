...
The Python SDK for the KokkaiKaigirokuApi API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Meeting(None).list(None, None)
if err:
    raise Exception(err)
...
result, err = client.KokkaiKaigirokuApi(None).load(
    {"id": "test01"}, None
)
