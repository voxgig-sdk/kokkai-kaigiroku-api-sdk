...
from kokkaikaigirokuapi_sdk.kokkaikaigirokuapi_types import (
    Speech,
    SpeechListMatch,
)
...
class SpeechEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return SpeechEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Speech]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Speech().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
