...
//    only - a typed KokkaiKaigirokuApiContext there would make every ctx
...
final class KokkaiKaigirokuApiOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new KokkaiKaigirokuApiContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new KokkaiKaigirokuApiSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new KokkaiKaigirokuApiResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new KokkaiKaigirokuApiError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new KokkaiKaigirokuApiResponse($clean['response']);
    ...
    public static function err_from_map($m): ?KokkaiKaigirokuApiError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new KokkaiKaigirokuApiError(is_string($code) ? $code : '', $msg);
    }
