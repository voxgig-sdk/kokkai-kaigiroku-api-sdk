...
describe("MeetingEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set KOKKAI_KAIGIROKU_API_TEST_MEETING_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local meeting_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.meeting")))
    local meeting_ref01_data = nil
    if #meeting_ref01_data_raw > 0 then
      meeting_ref01_data = helpers.to_map(meeting_ref01_data_raw[1][2])
    end
...
function meeting_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("KOKKAI_KAIGIROKU_API_TEST_MEETING_ENTID")
