...
local SpeechEntity = {}
SpeechEntity.__index = SpeechEntity


function SpeechEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SpeechEntity)
  self._name = "speech"
  self._client = client
...
function SpeechEntity:get_name()
  return self._name
end


function SpeechEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SpeechEntity.new(self._client, opts)
end
...
function SpeechEntity:mark_deleted()
  self._deleted = true
end


function SpeechEntity:deleted()
  return true == self._deleted
end


function SpeechEntity:data_set(args)
  if args ~= nil then
...
function SpeechEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SpeechEntity:match_set(args)
  if args ~= nil then
...
function SpeechEntity:match_get()
...
function SpeechEntity:stream(action, args, callopts)
...
function SpeechEntity:_run_op(ctx, post_done)
  local utility = self._utility
