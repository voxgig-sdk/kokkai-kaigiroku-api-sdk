...
The Lua SDK for the KokkaiKaigirokuApi API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Meeting(nil):list(nil, nil)
...
local result, err = client:KokkaiKaigirokuApi(nil):load(
  { id = "test01" }, nil
)
