...
require("kokkai-kaigiroku-api_types")
...
local KokkaiKaigirokuApiSDK = {}
KokkaiKaigirokuApiSDK.__index = KokkaiKaigirokuApiSDK
...
KokkaiKaigirokuApiSDK._make_feature = _make_feature


function KokkaiKaigirokuApiSDK.new(options)
  local self = setmetatable({}, KokkaiKaigirokuApiSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function KokkaiKaigirokuApiSDK:options_map()
...
function KokkaiKaigirokuApiSDK:get_utility()
  return Utility.copy(self._utility)
end


function KokkaiKaigirokuApiSDK:get_root_ctx()
  return self._rootctx
end


function KokkaiKaigirokuApiSDK:prepare(fetchargs)
...
function KokkaiKaigirokuApiSDK:direct(fetchargs)
...
function KokkaiKaigirokuApiSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function KokkaiKaigirokuApiSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "KokkaiKaigirokuApiSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function KokkaiKaigirokuApiSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function KokkaiKaigirokuApiSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "KokkaiKaigirokuApiSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function KokkaiKaigirokuApiSDK:MeetingList(data)
  ...
  if data == nil then
    if self._meeting_list == nil then
      self._meeting_list = EntityMod.new(self, nil)
    end
...
function KokkaiKaigirokuApiSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = KokkaiKaigirokuApiSDK.new(sdkopts)
  sdk.mode = "test"
