...
Learn more about 国会会議録検索API at [kokkai.ndl.go.jp](https://kokkai.ndl.go.jp).
...
This SDK exposes the API as a small set of **semantic entities** — Meeting, MeetingList and Speech — that you
...
const items = await client.Meeting().list()
```
...
| --- | --- | --- |
| **Meeting** | The Meeting entity (list). | `/meeting` |
| **MeetingList** | The MeetingList entity (list). | `/meeting_list` |
| **Speech** | The Speech entity (list). | `/speech` |
...
$meetings = $client->Meeting()->list();
print_r(array_map(fn($item) => $item->data_get(), $meetings));
```
