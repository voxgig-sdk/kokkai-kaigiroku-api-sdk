...
class SpeechDirectTest < Minitest::Test
  def test_direct_list_speech
    setup = speech_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-speech", setup[:live] ? "live" : "unit")
    if _should_skip
...
def speech_direct_setup(mockres)
