...
class MeetingListEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.MeetingList(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.MeetingList(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set KOKKAI_KAIGIROKU_API_TEST_MEETING_LIST_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def meeting_list_basic_setup(extra)
  ...
  entid_env_raw = ENV["KOKKAI_KAIGIROKU_API_TEST_MEETING_LIST_ENTID"]
