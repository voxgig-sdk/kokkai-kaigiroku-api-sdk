...
class MeetingDirectTest < Minitest::Test
  def test_direct_list_meeting
    setup = meeting_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-meeting", setup[:live] ? "live" : "unit")
    if _should_skip
...
def meeting_direct_setup(mockres)
