...
class MeetingListEntity
  ...
  def make
    opts = @_entopts.dup
    MeetingListEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = KokkaiKaigirokuApiHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [MeetingList, Hash] the current MeetingList data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = KokkaiKaigirokuApiHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of MeetingList fields)
  def match_get
  ...
  #
  # @param reqmatch [MeetingListListMatch, Hash, nil] match filter (any subset of
  #   MeetingList fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<MeetingList>, Array] the matching MeetingList items; raises KokkaiKaigirokuApiError on failure
