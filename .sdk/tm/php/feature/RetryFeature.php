...
// KokkaiKaigirokuApi SDK retry feature
...
class KokkaiKaigirokuApiRetryFeature extends KokkaiKaigirokuApiBaseFeature
{
    ...
    public function init(KokkaiKaigirokuApiContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (KokkaiKaigirokuApiContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(KokkaiKaigirokuApiContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(KokkaiKaigirokuApiContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
