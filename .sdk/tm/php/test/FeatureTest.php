...
// KokkaiKaigirokuApi SDK feature test
//
...
class FtCtrl extends KokkaiKaigirokuApiControl
{
...
class FtClient
{
    ...
    private KokkaiKaigirokuApiUtility $utility;

    public function __construct(KokkaiKaigirokuApiUtility $utility, array $options)
    {
    ...
    public function get_utility(): KokkaiKaigirokuApiUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (KokkaiKaigirokuApiContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public KokkaiKaigirokuApiContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (KokkaiKaigirokuApiContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = KokkaiKaigirokuApiConfig::make_feature($name);
    ...
    private static function buildUrl(KokkaiKaigirokuApiSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof KokkaiKaigirokuApiError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new KokkaiKaigirokuApiResponse($fetched);
        }
    ...
    private function fail(KokkaiKaigirokuApiContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (KokkaiKaigirokuApiContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
