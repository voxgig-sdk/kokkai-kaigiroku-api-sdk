...
module KokkaiKaigirokuApiUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = KokkaiKaigirokuApiResponse.new({})
    ...
    if fetch_err
      ...
      response = KokkaiKaigirokuApiResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = KokkaiKaigirokuApiResponse.new(fetched)
    else
