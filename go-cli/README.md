# kokkai-kaigiroku-api-cli

AQL-driven command-line client **and** interactive REPL for the KokkaiKaigirokuApi
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/kokkai-kaigiroku-api-cli)
make build

# 2. See usage (words, entities, env vars)
./kokkai-kaigiroku-api-cli --help
...
./kokkai-kaigiroku-api-cli list meeting
./kokkai-kaigiroku-api-cli list meeting_list

# 5. Override the API base URL for a single call
KOKKAI_KAIGIROKU_API_BASE=https://api.example.com ./kokkai-kaigiroku-api-cli list meeting

# 6. No arguments -> interactive REPL
./kokkai-kaigiroku-api-cli
kokkai-kaigiroku-api> list meeting
kokkai-kaigiroku-api> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/kokkai-kaigiroku-api-cli
   ```
...
3. **Run a query.** Evaluate an AQL expression against the API (or run with no
   ...
   ```sh
   ./dist/*/kokkai-kaigiroku-api-cli list meeting
   ```
...
Run with no arguments to open a REPL (prompt `kokkai-kaigiroku-api>`). Each line is
evaluated as its own AQL expression:

```text
$ ./kokkai-kaigiroku-api-cli
kokkai-kaigiroku-api> list meeting
kokkai-kaigiroku-api> /help
kokkai-kaigiroku-api> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an AQL atom (e.g. `meeting`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/kokkai-kaigiroku-api-cli`. |
...
(`./kokkai-kaigiroku-api-cli <expr>`) and interactively (the REPL), and expressions compose the
