// Command KokkaiKaigirokuApi-cli is a CLI + REPL for the KokkaiKaigirokuApi SDK,
...
const prompt = "kokkai-kaigiroku-api"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "meeting meeting_list speech"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      KOKKAI_KAIGIROKU_API_APIKEY (api key), KOKKAI_KAIGIROKU_API_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: KOKKAI_KAIGIROKU_API_APIKEY carries the API key and
	// KOKKAI_KAIGIROKU_API_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("KOKKAI_KAIGIROKU_API_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("KOKKAI_KAIGIROKU_API_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewKokkaiKaigirokuApiSDK(opts)
