...
describe('MeetingListDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new KokkaiKaigirokuApiSDK({
  ...
  test('direct-list-meeting_list', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-meeting_list', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['KOKKAI_KAIGIROKU_API_TEST_MEETING_LIST_ENTID']
