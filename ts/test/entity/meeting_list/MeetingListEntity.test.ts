...
describe('MeetingListEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (maybeSkipControl(t, 'entityOp', 'meeting_list.' + op, live)) return
    }
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set KOKKAI_KAIGIROKU_API_TEST_MEETING_LIST_ENTID JSON to run live')
      return
    }
    ...
    let meeting_list_ref01_data = Object.values(setup.data.existing.meeting_list)[0] as any
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/meeting_list/MeetingListTestData.json')
  ...
  const idmapEnvVal = process.env['KOKKAI_KAIGIROKU_API_TEST_MEETING_LIST_ENTID']
