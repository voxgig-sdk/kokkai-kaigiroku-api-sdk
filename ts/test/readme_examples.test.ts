...
//     (\`new KokkaiKaigirokuApiSDK(...)\` / \`KokkaiKaigirokuApiSDK.test(...)\` become
//     \`KokkaiKaigirokuApiSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'KokkaiKaigirokuApiSDK'
...
const TEST_SEED = {"entity":{"meeting":{"test01":{"id":"test01"}},"meeting_list":{"test01":{"id":"test01"}},"speech":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new KokkaiKaigirokuApiSDK(...)\` and any
// \`KokkaiKaigirokuApiSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+KokkaiKaigirokuApiSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/KokkaiKaigirokuApiSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
